-- Migration to add repeatable fields support to form schemas
-- This migration updates the Workers Compensation form to use proper fieldset-repeater fields

-- Update the form schema to add repeatable field definitions
UPDATE form_schemas 
...
-- Update sections to use the new repeater fields
UPDATE form_schemas 
SET form_schema = jsonb_set(
    form_schema,
    '{pages,2,sections}',
...
-- Add additional insureds section to page 5
UPDATE form_schemas 
SET form_schema = jsonb_set(
    form_schema,
    '{pages,5,sections,3}',
    '{
        "id": "section_additional_insureds",
        "type": "fieldset",
        "title": "Additional Insureds",
        "layout": "1-column",
        "items": [
            {"type": "field", "fieldId": "additional_insureds"}
        ]
    }'::jsonb
)
WHERE form_id = 'a1715135-94dc-456a-a68a-982ab0ed0fe1';

-- Clean up old individual fields that are now part of repeaters
UPDATE form_schemas 
SET form_schema = jsonb_set(
    form_schema,
    '{fields}',
...
SELECT 
    ...
    COUNT(*) FILTER (WHERE field_name IN ('vehicles', 'job_classifications', 'locations', 'additional_insureds', 'claims')) as repeater_fields_count
FROM (
